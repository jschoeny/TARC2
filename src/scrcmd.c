...
bool8 ScrCmd_messagecredits(struct ScriptContext *ctx)
{
    const u8 *msg = (const u8 *)ScriptReadWord(ctx);
    ...
    gTextFlags.isCredits = TRUE;
    ShowFieldCreditsMessage(msg);
    return FALSE;
}
...
bool8 ScrCmd_setsinglewildbattlecustom(struct ScriptContext *ctx)
{
    u16 species = ScriptReadHalfword(ctx);
    u8 level = ScriptReadByte(ctx);
    u16 item = ScriptReadHalfword(ctx);
    ...
    CreateScriptedWildMonCustom(species, level, item, nature, abilityNum, evs, ivs, moves, isShiny);
    sIsScriptedWildDouble = FALSE;
