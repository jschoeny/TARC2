...
static void ExpandStringAndStartDrawFieldMessage(const u8 *str, bool32 allowSkippingDelayWithButtonPress)
{
    if (gSpeakerName != NULL) {
        ...
        AddTextPrinterParameterized2(1, FONT_SMALL, gNamePlateBuffer, 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_TRANSPARENT, TEXT_COLOR_LIGHT_GRAY);
        PutWindowTilemap(1);
...
{
#if PORYLIVE
    gSpeakerName = PoryLive_GetScriptPointer(name);
#else
    gSpeakerName = name;
#endif
}
